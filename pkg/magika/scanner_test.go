...
func TestClient_Scan(t *testing.T) {
	...
	ct, err := c.Scan(context.Background(), bytes.NewReader(b), len(b))
	if err != nil {
		t.Fatal(err)
	}
	fmt.Println(util.ToJsonIndent(ct))
}
	...
	for _, e := range es {
		t.Run(e.Name(), func(t *testing.T) {
			...
			for _, ee := range es {
				...
				ct, err := s.Scan(context.Background(), bytes.NewReader(b), len(b))
				if err != nil {
...
func TestScannerSmall(t *testing.T) {
	...
	for _, c := range []struct {
		...
		t.Run(c.name, func(t *testing.T) {
			ct, err := s.Scan(context.Background(), bytes.NewReader(c.data), len(c.data))
			if err != nil {
				t.Fatalf("scan: %v", err)
			}
		...
		for _, pm := range []string{"high_confidence"} {
			t.Run(fmt.Sprintf("%s-%s", artifacts, pm), func(t *testing.T) {
				...
				for i, c := range tcases {
					...
					ct, score, err := s.scanScore(context.Background(), bytes.NewReader(c.Content), len(c.Content))
					if err != nil {
