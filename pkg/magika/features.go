...
func ExtractFeatures(cfg *ModelConfig, r io.ReaderAt, size int) (Features, error) {
	var (
...
func buildFeatures(cfg *ModelConfig, beg, mid, end []byte) Features {
	firstBlock := beg
...
func padInt32(cfg *ModelConfig, b []byte, prefix, size int) []int32 {
	r := make([]int32, 0, size)
	for len(r) < prefix {
