...
func TestGlobs(t *testing.T) {
	paths := []string{"../../tests_data/**"}
	absPaths := make([]string, len(paths))
	for i, p := range paths {
		if absPath, _ := filepath.Abs(path.Clean(ExpandPath(p))); absPath != "" {
			absPaths[i] = absPath
		}
	}
...
func TestGlobPatterns(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.pattern, func(t *testing.T) {
			matches, err := Globs(OS(), []string{tmpDir + test.pattern})
			require.NoError(t, err)
...
func TestGlobsAndSuperPaths(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.pattern, func(t *testing.T) {
			matches, err := GlobsAndSuperPaths(OS(), []string{tmpDir + test.pattern}, "js")
			require.NoError(t, err)
